...
public:
    ...
    void callback(const heron::Encoders& data)
    {
        ...
        speed.vx = MECANUM_X * (WHEEL_RADIUS) * (diff_encs.Fl + diff_encs.Fr + diff_encs.Bl + diff_encs.Br)/4;        // m/s
        speed.vy = MECANUM_Y * (WHEEL_RADIUS) * (- diff_encs.Fl + diff_encs.Fr - diff_encs.Bl + diff_encs.Br)/4;      // m/s
